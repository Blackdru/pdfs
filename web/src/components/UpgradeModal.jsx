...
const UpgradeModal = ({ isOpen, onClose, requiredPlan = 'pro', toolName = '', toolDescription = '' }) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-grey-900 border-grey-800">
        <DialogHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              size="icon"
              onClick={onClose}
              className="h-6 w-6 text-grey-400 hover:text-grey-200"
            >
              <X className="h-4 w-4" />
            </Button>
          </div>
        ...
        <div className="space-y-6">
          {/* Tool Info */}
          {toolName && (
            <div className="bg-grey-800 rounded-xl p-6 border border-grey-700">
              <div className="flex items-center space-x-3 mb-3">
                <div className="w-8 h-8 bg-gradient-to-r from-purple-500 to-pink-500 rounded-lg flex items-center justify-center">
                  <Sparkles className="h-4 w-4 text-white" />
                </div>
                <h3 className="text-lg font-semibold text-grey-100">{toolName}</h3>
              ...
              {toolDescription && (
                <p className="text-grey-400 mb-4">{toolDescription}</p>
              )}
          ...
          <div className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 rounded-xl p-6 border border-purple-800/30">
            <h3 className="text-lg font-semibold text-grey-100 mb-4 flex items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {recommendedPlans.map((plan) => (
              <div
                key={plan.id}
                className={`relative bg-grey-800 rounded-2xl border-2 transition-all duration-300 hover:scale-105 ${
                  plan.id === 'pro' 
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="text-center mb-6">
                    <h3 className="text-xl font-bold text-grey-100 mb-2">{plan.name}</h3>
                    <div className="text-3xl font-bold text-grey-100 mb-1">
                      ${plan.price}
                      <span className="text-sm text-grey-400 font-normal">/month</span>
                    </div>
          ...
          {/* Footer */}
          <div className="text-center space-y-4 pt-6 border-t border-grey-800">
            <div className="flex items-center justify-center space-x-6 text-sm text-grey-400">
